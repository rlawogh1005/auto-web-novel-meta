...
# Domain Model — 회차 생성·검수 + rewrite 루프 + AI 독자 댓글 + 작가-PD 페어링 줄기

> 이 문서는 `auto-web-novel` 시스템의 도메인 중 **회차 1건이 생성되어 검수를 거치는 과정**(walking skeleton 1단계), **reject 시 generator 가 피드백을 받아 본문을 재작성하는 rewrite 루프**(walking skeleton 3단계), **발행된 회차에 AI 독자가 댓글을 다는 과정**(walking skeleton 2단계), **각 작가에게 전담 PD 1명이 배정되는 1:1 페어링 골격**(walking skeleton 4단계)에 등장하는 개념·규칙만을 다룬다.
> 사람 UI(좋아요·댓글), 독자 가상결제, admin UI, **PD 별 정체성 / 시너지(담당 PD 의 작가 이력 검수 입력 주입)** 등은 별도 명세에서 다룬다.
...
| `assigned_pd_id` | 이 Novel을 검수하는 `PdAgent`. Novel 활성화 시점에 배정되어 종착(`published`/`archived`)까지 불변 (§4.11). |
...
| `number` | 회차 번호(1, 2, 3, …). 같은 Novel 내 유일. `abandoned` 종착 시 번호 갭 발생 가능 (§4.10). |
| `title` | 회차 제목. |
| `content` | 본문(마크다운). rewrite 시 갱신(SRS-F-007). |
| `status` | `draft` \| `in_review` \| `published` \| `abandoned` \| `rejected`. `rejected` 는 본 줄기 미사용(§4.1), `abandoned` 는 재시도 상한 종착(§4.10). 이전 명세의 `approved` 는 본 개정에서 제거 (구현 정합화). |
| `revision_count` | int. reject/needs_revision 시 +1 (§4.6). MAX 도달이면 abandoned 종착 (§4.10). |
| `abandoned_at` | abandoned 전이 시각. published_at 패턴 대칭. |
...
한 Chapter는 여러 Review를 가질 수 있다(reject/needs_revision → draft → rewrite → 다시 in_review → 재검수 흐름이 §4.10 의 MAX 미만 동안 반복될 수 있으므로).

### PdAgent

회차를 검수하는 "PD"의 명부 엔티티. Writer 와 패턴이 유사하나 **정체성 파일(SOUL 등)이 없다** — 본 줄기에서 PD 는 공통 rubric (SRS-F-003 (A)) 으로 동작하므로 인스턴스별 변별이 없다. 명부는 식별자와 활성 여부만 보유한다.

| 속성 | 설명 |
|---|---|
| `id` | PD 식별자 슬러그 (예: `pd-alpha`). 불변. |
| `active` | 현재 활동 중 여부. 비활성 PD 에게는 새 Novel 을 배정하지 않는다. |
| `created_at` | PD 가 시스템에 등록된 시각 (UTC). |

**생명주기**: 사람이 시스템에 등록한다. 본 줄기에서 정체성 본문이 없으므로 Writer / ReaderPersona 와 달리 파일 디렉토리 참조(`identity_ref`)를 가지지 않는다. 향후 PD 별 정체성(SOUL · 검수 관점) 이 필요해지면 그때 `identity_ref` 를 추가한다 (Navigator 빚).

> **검수 결과(Review) 와의 관계**: 본 줄기에서 `Review` 는 `pd_version`(빌드 식별자) 만 영속하고 **어느 PdAgent 가 수행했는지는 영속하지 않는다** — `pd.reviews` 테이블 스키마는 본 줄기에서 변경하지 않는다 (분석 추적은 후속 빚). Novel ↔ PdAgent 페어링(§4.11)이 정해져 있으므로 `Review.chapter_id → Chapter.novel_id → Novel.assigned_pd_id` 경유로 사후 추적은 가능하나, **Novel 도중 PD 재배정이 도입되면 이 경유 추적이 무너진다** — 그때 `pd.reviews` 에 `pd_id` 컬럼 추가 필요 (Navigator 빚).
...
- **외부 참조**: Novel은 자신을 쓰는 Writer를 1개 참조(`writer_id`) 하고 자신을 검수하는 PdAgent 를 1개 참조(`assigned_pd_id`) 한다. Writer 와 PdAgent 는 각각 별도 집합체이며 Novel 집합체에 포함되지 않는다 — WriterIdentity 의 본문이 파일에 있고 DB 가 소유하지 않기 때문이며, PdAgent 는 본 줄기에서 정체성 본문이 없지만 명부 자체가 PD 서비스의 소유라서 마찬가지다.
- **불변식**: §4의 모든 도메인 규칙은 이 집합체 안에서 보장된다. Writer↔Novel 카디널리티는 §4.7, Novel↔PdAgent 카디널리티는 §4.11 에서 별도로 강제한다.
...
- **포함**: 단일 Review.
- **소유 서비스**: `pd`.
- Review는 Chapter를 참조(`chapter_id`)하지만 Chapter 집합체에 포함되지 않는다. pd는 Novel 집합체 내부 상태(WriterContext 등)를 변경하지 않고, 오직 Chapter.status·`revision_count`·(reject/needs_revision 시) feedback_log 누적에만 영향을 준다.

### Aggregate "PdAgent"
- **루트**: `PdAgent`
- **포함**: 단일 PdAgent.
- **외부 참조**: 본 줄기에서 없음 (정체성 파일이 없으므로 `identity_ref` 도 없다). 향후 PD 정체성 도입 시 추가.
- PdAgent 는 Writer 와 같은 이유로 별도 집합체 루트다 — PD 서비스 소유의 명부이며 generator/viewer 가 직접 수정하지 않는다. admin 이 시드하고 pd 는 자기 식별자(`$self_pd_id`) 로만 자신을 인지한다.
...
```
     [draft] ──── generator 완성 판정 ────▶ [in_review] ── pd: approve ──▶ [published]
        ▲                                       │                          (종착·성공)
        │                                       │
        │ pd: reject / needs_revision           │ pd: reject / needs_revision
        │ (revision_count++, 결과 < MAX)         │ (revision_count++, 결과 >= MAX)
        │                                       │
        └───────────────────────────────────────┴────────▶ [abandoned]
                                                          (종착·실패, tombstone)
```
...
**전이 규칙**:
- 허용 전이: `draft → in_review`, `in_review → published`, `in_review → draft`, `in_review → abandoned`.
- `published`(성공)와 `abandoned`(실패) 가 본 줄기의 종착 상태다. 종착 상태에서는 더 이상 전이가 없다.
- `in_review → draft` 와 `in_review → abandoned` 는 동일한 pd 의 reject/needs_revision 결정으로 발생하며, 분기는 같은 트랜잭션 안에서 `revision_count` 증가 결과로 결정된다 — §4.10 (재시도 상한). MAX 미만이면 `draft` 로 복귀, MAX 이상이면 `abandoned` 로 종착.
- 위 외의 전이는 거부된다 (예: `draft → in_review` 외의 모든 draft 진출, `published`/`abandoned` 에서의 모든 진출, `draft → abandoned` 직접 전이 등).
- `rejected` 는 상태 enum 에 존재하지만 본 줄기에서는 사용하지 않는다. reject 결정은 `pd.reviews.decision='reject'` 에 기록되고 Chapter.status 는 위 분기에 따라 `draft` 또는 `abandoned` 로 전이된다. enum 값 제거는 후속 빚 (Navigator 기록).
- 이전 명세에 있던 `in_review → approved → published` 중간 단계 `approved` 는 본 개정에서 제거됐다 — 1c 구현 정합화 (Navigator "기술 빚" 해소).

**전이를 수행하는 주체**:
- `draft → in_review`: generator (SRS-F-002).
- `in_review → published`: pd (SRS-F-004, approve 결정).
- `in_review → draft`: pd (SRS-F-004, reject/needs_revision 결정 + `revision_count` < MAX).
- `in_review → abandoned`: pd (SRS-F-008, reject/needs_revision 결정 + `revision_count` >= MAX).

**rewrite 루프**:
- `in_review → draft` 복귀 후 generator 가 같은 Chapter row 를 발견해 본문을 재작성한다 (SRS-F-007). 회차 번호는 그대로, `content` 만 갱신, `revision_count` 는 pd 가 다음 reject 시 다시 증가시킨다.
- 같은 Novel 내 다른 회차로 진행하기 위해 generator 는 active draft(§4.3) 가 없어야 한다. `abandoned` 종착 후에는 active draft 가 없으므로 다음 cycle 에 N+1 fresh 진행이 가능하다 — 회차 번호 갭이 발생한다 (§4.10).

### 4.2 회차 번호 단조 증가
같은 `novel_id` 내 `Chapter.number`는 1부터 빈틈없이 단조 증가한다. 새 회차는 항상 `max(number) + 1`로 만든다.

### 4.3 한 번에 한 회차
같은 `novel_id` 내 `status ∈ {draft, in_review}`인 Chapter는 동시에 최대 1개 존재한다. 이전 회차가 `published` 또는 `abandoned` 로 종착하거나 다시 `draft`로 돌아온 뒤라야 다음 회차(N+1) fresh 생성이 가능하다. `draft` 복귀의 경우 N+1 신규 생성이 아니라 같은 회차의 rewrite(SRS-F-007)로 이어진다.
...
판정 적용 범위: 본 §4.5 의 완성 판정은 generator 의 **fresh 모드** (새 회차 생성, SRS-F-001) 와 **rewrite 모드** (기존 draft 본문 재작성, SRS-F-007) 양쪽에 동일하게 적용된다 — Chapter.status 가 `draft` 인 한 다음 `in_review` 전이의 자격 검사는 항상 §4.5 다.

### 4.6 피드백 누적
Review.decision이 `reject` 또는 `needs_revision`일 때, 해당 Review.feedback은 WriterContext.feedback_log에 누적된다. 이 누적은 Chapter.status를 `in_review` 에서 다음 상태(`draft` 또는 `abandoned`)로 전이하는 동일 트랜잭션 내에서 일어난다(부분 적용 금지).

같은 트랜잭션 안에서 Chapter.`revision_count` 가 1 증가한다 (Data Model §1). 증가 결과가 §4.10 의 MAX 이상이면 §4.1 분기에 따라 Chapter.status 는 `draft` 가 아닌 `abandoned` 로 종착한다. 어느 경우든 feedback_log 누적은 동일하게 일어난다 (abandoned 회차의 누적도 보존돼 사후 분석에 사용).
...
Comment는 `Chapter.status='published'` 인 회차에만 달릴 수 있다. 다른 상태(`draft`, `in_review`, `abandoned`)의 Chapter에 대한 Comment INSERT는 거부된다. 강제 위치는 Data Model의 트리거 또는 애플리케이션 레이어 (`[확인 필요]`).
...
### 4.10 재시도 상한 + abandoned 종착

무한 rewrite 루프를 방지하기 위해 한 Chapter 의 reject 횟수에 상한 `MAX_REVISION_ATTEMPTS` 를 둔다.

- **MAX 값**: `[확인 필요 — 기본 3 권장]` (너무 작으면 정상적인 가차없는 검수가 abandoned 폭증, 너무 크면 비용·시간 낭비).
- **트리거**: pd 가 reject 또는 needs_revision 결정으로 §4.6 트랜잭션을 수행할 때, 같은 트랜잭션 안에서 Chapter.`revision_count` 가 1 증가한 결과가 MAX 이상이면 §4.1 의 분기 `in_review → abandoned` 가 선택된다 (`in_review → draft` 대신).
- **abandoned 의 의미**:
  - 해당 회차는 폐기된다. 본 줄기에서 다시 활성 상태(`draft`/`in_review`)로 돌아가지 않는다 — 종착이다.
  - row 는 tombstone 으로 보존된다 (DELETE 하지 않음). pd.reviews 누적, draft_runs 메타, feedback_log 누적 모두 사후 분석 자료로 남는다.
  - `abandoned_at` 컬럼이 종착 시각으로 기록된다 (Data Model §1).
- **후속 회차 진행**: abandoned 는 §4.3 의 "active 회차(`draft` 또는 `in_review`)" 정의에 들지 않으므로, 다음 generator cycle 에서 N+1 fresh 회차 생성(SRS-F-001)이 가능하다. **회차 번호 갭이 발생할 수 있다** (예: 5 abandoned → 다음은 6 으로 진행, 발행 회차 목록에 5번이 빠짐).
- **운영자 가시성 / 개입 모델은 본 줄기 범위 밖**: abandoned 발생 알림, admin UI 노출, Novel 자체의 일시중지 정책 등은 별도 명세 영역 (Navigator 신규 빚으로 기록).

### 4.11 Novel ↔ PdAgent 카디널리티 / 1:1 페어링

- PdAgent ↔ Novel = **1:N** (이력 차원). 한 PdAgent 는 여러 Novel 을 이력으로 가질 수 있다.
- **한 PdAgent 가 동시에 `Novel.status='drafting'` 인 Novel 은 최대 1개** — Writer ↔ Novel (§4.7) 와 패턴 대칭. active 의 정의는 §4.7 의 `Novel.status='drafting'` 단일 기준을 재사용한다.
- Writer ↔ Novel (§4.7) 와 결합되어 결과적으로 (Writer, Novel, PdAgent) 가 동시에 active 인 동안 **1:1:1 페어** 가 된다. 본 명세에서 페어링의 단위는 Novel 이며, Writer 와 PdAgent 는 같은 Novel 을 통해 1:1 로 묶인다.
- 페어링은 **Novel 활성화 시점 (`status='drafting'` 으로 진입할 때)** 에 형성되고 Novel 종착(`published` / `archived`) 까지 불변이다. Novel 도중 `assigned_pd_id` 재배정은 본 줄기 범위 밖 (Navigator 빚).
- **불변식 강제 위치**: Data Model 의 부분 유니크 인덱스 `novels_one_active_per_pd` (Data-Model.md §4.1) — `novels_one_active_per_writer` 와 대칭.
- **배정 주체·알고리즘**: `[확인 필요 — 코드 PR 의 ADR]`. 본 명세는 "Novel 활성화 시점에 active=true PdAgent 1명이 배정된다" 까지만 강제하며, 자동 round-robin / 랜덤 / 수동(admin) / 시드 중 무엇으로 배정할지는 결정하지 않는다.
- **PD 명부 부족 케이스** (PD 0명 또는 active Novel 수보다 적음): 본 줄기 범위 밖. 명세 수준에서 기본 동작은 — `pd.pd_agents` 에 active PD 가 없으면 `assigned_pd_id` NOT NULL FK 위반으로 Novel 생성이 거부된다. 운영 정책(대기열 / 알림 / 수동 배정 / Novel 일시중지) 은 Navigator 빚.
- **시너지 (담당 PD 의 그 작가 이력 검수 입력 주입)** 는 본 §4.11 의 페어링 골격이 만든 다리 위에서만 가능해진다. 본 줄기에서는 다리만 놓고 (정체성·이력 주입은 다음 줄기) — Navigator 빚.
...
| 이벤트 | 발생 조건 | 발화 주체 |
|---|---|---|
| `ChapterDrafted` | 새 Chapter row가 `status=draft`로 삽입됨 (fresh 모드, SRS-F-001) | generator |
| `ChapterRewritten` | 기존 draft Chapter 의 `content` 가 갱신됨 (rewrite 모드, SRS-F-007). status 변경 없음 | generator |
| `ChapterSubmittedForReview` | Chapter.status가 `draft → in_review` 전이 | generator |
| `ChapterPublished` | Chapter.status가 `in_review → published` 전이 (approve 결정, SRS-F-004) | pd |
| `ChapterRejected` | Chapter.status가 `in_review → draft` 전이 + feedback 누적 + `revision_count` 증가 (결과 < MAX) | pd |
| `ChapterAbandoned` | Chapter.status가 `in_review → abandoned` 전이 + feedback 누적 + `revision_count` 증가 (결과 >= MAX, §4.10) | pd |
