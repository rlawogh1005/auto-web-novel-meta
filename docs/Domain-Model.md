...
# Domain Model — 회차 생성·검수 + rewrite 루프 + AI 독자 댓글 줄기

> 이 문서는 `auto-web-novel` 시스템의 도메인 중 **회차 1건이 생성되어 검수를 거치는 과정**(walking skeleton 1단계), **reject 시 generator 가 피드백을 받아 본문을 재작성하는 rewrite 루프**(walking skeleton 3단계), **발행된 회차에 AI 독자가 댓글을 다는 과정**(walking skeleton 2단계)에 등장하는 개념·규칙만을 다룬다.
