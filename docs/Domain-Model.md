...
### ReaderIdentity (향후 — 개념만)

`AgentIdentity` + 읽기·댓글 스타일. viewer 줄기의 reader-agents가 사용할 정체성이다. 본 줄기(회차 생성→검수) 범위 밖이며 viewer 명세에서 구체화된다. 여기서는 AgentIdentity가 작가뿐 아니라 독자에게도 공통 베이스가 됨을 명시하는 용도로만 언급한다.
