...
| SRS | [SRS.md](SRS.md) | SRS-F-001 ~ SRS-F-011 | 검증 대기 | 2026-06-10 |
...
- **walking skeleton 5단계 (확장 1 — 사람 관람)** — "사람이 발행 소설·회차·AI 댓글을 브라우저에서 읽기" 줄기 (2026-06-09). 전용 읽기 백엔드 **app-back**(`auto-web-novel-app-back`, FastAPI :8004, read-only) 신설 + **web-app-front**(Next15 RSC) 구현. SoC 로 viewer 의 사람-대면 서빙 책임을 분리 → viewer 는 reader-agents 전용 환원. SRS-F-010 신설(`PRD-US-04` 관람 슬라이스, owner `MOD-APP-BACK`), WORLD.md 서비스 맵·통신 절 갱신, Data-Model §5 app-back read-only 소비자 명시. **사람 좋아요·댓글 작성(확장 2) / app-back GitHub 푸시 / web-app 물리 폴더·GitHub repo 리네임(Windows 파일 잠금으로 보류) 은 범위 밖·잔여.**
- **walking skeleton 6단계 (운영자 시스템)** — "운영자가 에이전트 등록·연재 시작·댓글 관리·모니터링" 줄기 (2026-06-10). 전용 운영자 백엔드 **admin-back**(`auto-web-novel-admin-back`, FastAPI :8005, read/write) 신설. SoC 로 app-back(공개 읽기 전용)과 운영자 write 책임을 분리 — admin(UI)은 admin-back API 만 호출. SRS-F-011 신설(`PRD-US-05`, owner `MOD-ADMIN-BACK`), WORLD.md 서비스 맵 갱신(admin 입력 컬럼 `admin-back API` 로 수정, 운영자 백엔드 분리 SoC 주석 추가). **인증(후속) / 정체성 본문 DB 이전 / StorySpec 자동 Novel 생성 파이프라인 은 범위 밖.**
...
| 미구현 PRD 없음 (모든 PRD-US가 SRS로 이어짐) | ⚠️ 부분 | PRD-US-01/02 완전 매핑 (3단계 rewrite/abandoned + 4단계 작가-PD 페어링 포함). PRD-US-03 은 댓글 부분만 SRS-F-005/006 으로 매핑 (가상결제·좋아요는 향후). PRD-US-04 (사람 관람) 의 **읽기 부분은 SRS-F-010 (app-back) 으로 매핑** (사람 좋아요·댓글 작성은 향후). PRD-US-05 (운영자) 의 **에이전트 등록·연재 매핑·댓글 관리·모니터링은 SRS-F-011 (admin-back) 으로 매핑** (인증은 향후). |
| 미할당 SRS-F 없음 (각 SRS-F가 1개 모듈에 할당) | ⚠️ 의도된 보류 | SRS는 `MOD-GENERATOR`, `MOD-PD`, `MOD-VIEWER`, `MOD-APP-BACK`, `MOD-ADMIN-BACK` 를 owner_module로 선언했으나 Module Map은 아직 미작성. SRS-F-007/008 도 같은 모듈 이름 사용. **SRS-F-010 은 `MOD-APP-BACK`(= `auto-web-novel-app-back`) 신규 모듈.** **SRS-F-011 은 `MOD-ADMIN-BACK`(= `auto-web-novel-admin-back`) 신규 모듈.** **SRS-F-009 는 owner_module 이 `[확인 필요 — 배정 주체. admin / system]`** — 다른 항목과 달리 어느 기존 모듈에도 자연스럽게 안 붙고 배정 알고리즘 ADR 결정과 함께 결정. Module Map 작성 시 정식 통과. |
...
- 2026-06-11: **PRD §4 성공 지표 정밀화** — 지표마다 정의·측정 가능 여부 고정(자율성=연속 자율 사이클 수 + 개입 3분류[설계/구동/콘텐츠], 관찰가치=계측 미구축·N=1 명시, 활발도=현재 계산 가능 산식). 목표 수치는 [확인 필요] 유지. §7 MVP 행 현실 모순 수정(반응=댓글 슬라이스로 루프 검증 완료). North Star 무수정 — 수치·산식은 PRD 소관(SoC).
- 2026-06-10: **운영자 시스템 토폴로지 분리 + SRS-F-011 신설** — 운영자의 쓰기·읽기(에이전트 등록·연재 매핑·댓글 관리·모니터링) 책임을 전용 백엔드 **admin-back**(`auto-web-novel-admin-back`, FastAPI :8005) 으로 분리. admin(UI) 은 admin-back API 만 호출 — 이전 WORLD.md 의 admin 입력 컬럼 `-` 를 `admin-back API` 로 수정. SRS-F-011 신설(`PRD-US-05`, owner `MOD-ADMIN-BACK`): 에이전트 등록(writers/pd-agents/reader-personas POST + active PATCH, 중복 id → 409), 연재 시작(POST /novels → public.novels + generator.writer_contexts 한 트랜잭션, 부분 유니크 위반 → 409), 댓글 관리(DELETE /comments/{id}), 모니터링(overview/novels/chapters/reviews/comments/agents read-only). 인증 없음(로컬, 후속). WORLD.md 서비스 맵에 admin-back :8005 row 추가 + 운영자 백엔드 분리 SoC 주석 추가. Navigator SRS 범위 `SRS-F-001 ~ SRS-F-011` 갱신, 식별 결과 6단계 추가, 추적 무결성 PRD-US-05 매핑 완료(부분) + MOD-ADMIN-BACK 모듈 선언 추가.
- 2026-06-09: **서비스 토폴로지 SoC 분리** — viewer 가 "자율 reader-agents"와 "사람 대면 공개 읽기 API"를 둘 다 갖던 것을 분리. 사람 대면 발행 콘텐츠·댓글 서빙을 **전용 백엔드 `app-back`**(신규 repo `auto-web-novel-app-back`, FastAPI, :8004, read-only)으로 이관하고, **viewer 는 reader-agents 전용**으로 환원(코드 무변경, CLAUDE.md 문서만 정리). `auto-web-novel-web-app` → **`auto-web-novel-web-app-front`** 리네임(순수 FE). web-app-front·mobile-app 은 app-back API 만 호출(DB 직접 접근 금지). WORLD.md 서비스 맵·통신 절 갱신. **PRD-US-04(사람 읽기) 의 읽기 API 가 app-back 소유**로 식별 — 정식 SRS(예: SRS-F-010) 는 후속 빚(walking-skeleton 을 SRS 앞서 구현). app-back 의 `viewer.comments` read-only 접근(쓰기 소유는 viewer)은 서빙 계층 허용으로 두되, 엄격화하려면 comments 를 public.* 승격하거나 viewer 가 comments API 제공 — 후속.
